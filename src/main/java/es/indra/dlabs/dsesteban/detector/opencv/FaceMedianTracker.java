 ...
 */
@Singleton
public class FaceMedianTracker {

    /** Logger. */
    private static final Logger LOG = LoggerFactory.getLogger(FaceMedianTracker.class);

    private static final String ID = "Tracker - Median";

    private static final Duration LAPSUS = Duration.ofMillis(80);
    ...
    @PostConstruct
    void init() {
        timer = registry.timer("tracker.median");
    }
    ...
    public void processImage(@ObservesAsync @GrabberEvent final OpenCVVideoFrame frame) {
        if (initialized && active && tracking) {
            ...
            if (Duration.between(lastProcessed, now).compareTo(LAPSUS) > 0) {
                ...
                // TODO: si pierde la imagen hay que contar reintentos de volver a encontrarla
            }
        }
    }
